...
from alma_tv.database import PlayHistory, Session, Video
from alma_tv.database.models import SessionStatus
from alma_tv.scheduler.lineup import LineupGenerator


@pytest.fixture
def scheduler_settings(test_settings):
    """Override scheduler settings."""
    test_settings.target_duration_minutes = 30
    test_settings.repeat_cooldown_days = 14
    return test_settings


@pytest.fixture
def sample_videos(db_session, scheduler_settings):
...
def test_generate_lineup_basic(mock_duration, sample_videos, db_session):
...
@patch("alma_tv.scheduler.lineup.LineupGenerator._get_file_duration")
def test_lineup_runtime_target(mock_duration, sample_videos, db_session):
    ...
    # Should be within ±90s
    assert abs(total_duration - 1780) <= 90


@patch("alma_tv.scheduler.lineup.LineupGenerator._get_file_duration")
def test_lineup_diversity(mock_duration, sample_videos, db_session):
...
@patch("alma_tv.scheduler.lineup.LineupGenerator._get_file_duration")
def test_lineup_handles_requests(mock_duration, sample_videos, db_session):
...
@patch("alma_tv.scheduler.lineup.LineupGenerator._get_file_duration")
def test_lineup_prevents_duplicates(mock_duration, sample_videos, db_session):
...
@patch("alma_tv.scheduler.lineup.LineupGenerator._get_file_duration")
def test_lineup_respects_cooldown(mock_duration, sample_videos, db_session):
    ...
    session = db_session.query(Session).filter(Session.id == session_id1).first()
    ...
    session1 = db_session.query(Session).filter(Session.id == session_id1).first()
    session2 = db_session.query(Session).filter(Session.id == session_id2).first()
...
@patch("alma_tv.scheduler.lineup.LineupGenerator._get_file_duration")
def test_lineup_episode_count_bounds(mock_duration, sample_videos, db_session):
...
@patch("alma_tv.scheduler.lineup.LineupGenerator._get_file_duration")
def test_lineup_deterministic_with_seed(mock_duration, sample_videos, db_session):
    ...
    # Clear database (must clear PlayHistory first due to FKs)
    db_session.query(PlayHistory).delete()
    db_session.query(Session).delete()
    db_session.commit()

    # Generate second lineup
